...
- P1 (CCE базовый): добавлены миграции/репозитории для brand/product/snippets, сервис `CCEService` (CRUD + BuildPromptContext), новые API (`PUT /v1/projects/:id/brand-profile`, `/product-profile`, `/snippets`, `GET /context`), а также интеграция в router/main. Тесты (`cd apps/backend && go test ./...`) покрывают базовый сценарий.
