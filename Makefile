...
migration: ## Create new migration (use: make migration name=add_users_table)
	...
	@echo ""
	@echo "Don't forget to run: make migration-checksums"

migration-checksums: ## Generate checksums for migrations (run after creating new migration)
	@echo "Generating migration checksums..."
	@cd apps/backend/migrations && \
		echo "# Migration checksums - DO NOT EDIT MANUALLY" > checksums.sha256 && \
		echo "# Generated by: make migration-checksums" >> checksums.sha256 && \
		echo "# Purpose: Ensure migrations are immutable after deployment" >> checksums.sha256 && \
		echo "#" >> checksums.sha256 && \
		echo "# If CI fails on checksum validation:" >> checksums.sha256 && \
		echo "# 1. NEVER modify an already-applied migration" >> checksums.sha256 && \
		echo "# 2. Create a new migration instead" >> checksums.sha256 && \
		echo "# 3. Run: make migration-checksums" >> checksums.sha256 && \
		echo "#" >> checksums.sha256 && \
		shasum -a 256 *.sql >> checksums.sha256
	@echo "Checksums updated: apps/backend/migrations/checksums.sha256"

migration-validate: ## Validate migration checksums (used in CI)
	@echo "Validating migration checksums..."
	@cd apps/backend/migrations && \
		grep -v "^#" checksums.sha256 | grep -v "^$$" | shasum -a 256 -c -
	@echo "All migration checksums valid!"

migration-status: ## Show pending migrations (dry-run)
	@echo "Checking migration status..."
	@docker-compose -f deploy/docker/docker-compose.yml exec -T backend /app/goose -dir /app/migrations postgres "$${DSN:-postgres://landly:landly@postgres:5432/landly?sslmode=disable}" status

migration-dry-run: ## Show what migrations would be applied
	@echo "Migrations that would be applied:"
	@docker-compose -f deploy/docker/docker-compose.yml exec -T backend /app/goose -dir /app/migrations postgres "$${DSN:-postgres://landly:landly@postgres:5432/landly?sslmode=disable}" status | grep -E "Pending|Applied"
