...
func (s *PublishService) PublishSite(ctx context.Context, userID, projectID string, req *domain.PublishRequest) (*domain.PublishTarget, error) {
	...
	// Формат: https://landly.com/{username}
...
func (s *PublishService) GetPublishedURL(ctx context.Context, userID, targetID string) (string, error) {
	...
	return s.publisher.GetPublicURL(target.Subdomain), nil
}

// UnpublishSite отменяет публикацию
func (s *PublishService) UnpublishSite(ctx context.Context, userID, targetID string) error {
	target, err := s.GetPublishStatus(ctx, userID, targetID)
	if err != nil {
		return err
	}
	...
	return s.publishTargetRepo.Update(ctx, target)
}
...
func (s *PublishService) PublishProject(ctx context.Context, userID, projectID uuid.UUID) (*PublishResult, error) {
	...
	// Генерируем публичный URL в формате https://landly.com/{username}
	// Всегда используем формат с username, независимо от cdnBase
	publicURL := fmt.Sprintf("https://landly.com/%s", username)
