...
type PublishService interface {
	...
	UnpublishSite(ctx context.Context, userID, targetID string) error
}
...
func NewGenerateHandler(generateService GenerateService, publishService PublishService) *GenerateHandler {
...
func (h *GenerateHandler) Publish(c *gin.Context) {
	...
	// Формируем URL в формате https://landly.com/{username}
	// Получаем username из email пользователя
	username := userID.String()[:8] // Временно используем часть ID, пока нет отдельного поля
